...
The app can generate deterministic render jobs with complete scene descriptions and machine asset references by running `npm run render -- --engine <blender-cycles|luxcore|mitsuba-3> --resolution 2k`. The Node server exposes the same flow at `POST /api/render`; if Blender, LuxCoreRender, or Mitsuba is installed on the host, the render script invokes the matching executable, otherwise it writes the deterministic scene files under `artifacts/render-jobs/` and reports the missing renderer. Machine CAD proxy assets live in `assets/machines/` and can be regenerated with `npm run assets`.
