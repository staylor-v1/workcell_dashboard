...
The app can generate deterministic render jobs with complete scene descriptions and machine asset references by running `npm run render -- --engine <blender-cycles|luxcore|mitsuba-3> --resolution 2k --execute true`. The Node server exposes the same flow at `POST /api/render`, and Vite's `npm run dev` server wires that API in through `vite.config.js`; you do not need to run an additional app rebuild after `npm run build` for the Render button to work. If Blender, LuxCoreRender, or Mitsuba is installed on the host, the render script invokes the matching executable. Otherwise it writes deterministic scene files under `artifacts/render-jobs/` and reports the missing renderer without producing PNG review images. For successful image renders, install the selected renderer or set `MICROFACTORY_BLENDER_BIN`, `MICROFACTORY_LUXCORE_BIN`, or `MICROFACTORY_MITSUBA_BIN` to the full executable path before starting `npm run dev` or `npm run server`; restart the server after changing those environment variables. Mitsuba jobs are rendered through the installed Mitsuba Python module so pip-installed Mitsuba 3 can write the expected PNG review images reliably; when `mitsuba` is a Python launcher, the worker now detects and uses that launcher's shebang Python interpreter before falling back to the generic `python3`, and you can still set `MICROFACTORY_PYTHON_BIN` if the server should use a specific Python environment. On Linux, the generated Mitsuba driver auto-detects common `libLLVM` install paths before importing Mitsuba/Dr.Jit. On Windows, renderer discovery checks the server process `PATH` and `PATHEXT` directly instead of relying on a Unix shell, so executables that work from PowerShell are detected when the server is launched from that environment. Machine CAD proxy assets live in `assets/machines/` and can be regenerated with `npm run assets`.
